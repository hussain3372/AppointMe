...
const ActionDropdown = ({ 
  ...
  row: any; 
  actions: (row: any, closeDropdown: () => void) => React.ReactNode 
}) => {
      ...
      {isDropdownOpen && (
        <>
          ...
          <div className="absolute right-0 top-full mt-1 w-48 bg-white rounded-md shadow-lg border border-gray-200 z-20">
            {actions(row, () => setIsDropdownOpen(false))}
          </div>
        </>
      )}
    </div>
...
export default function SideContent() {
   ...
   const getActions = (row: any, closeDropdown: () => void): React.ReactNode => (
  <div className="p-2">
    <button 
      type="button"
      ...
      onClick={() => {
        ...
        closeDropdown(); // Use the passed function
      }}
    >
      Send Email
    </button>
    {/* Add closeDropdown() to all other buttons similarly */}
    <button 
      type="button"
    ...
    return (
        <div className='pt-5 h-full flex flex-col'>
                    ...
                    <div className='flex flex-col items-center pt-[32px] flex-1'>
                        ...
                        <div className='pt-8 w-full flex-shrink-0 px-4'>
                            <div className='flex justify-between'>
                                <p className='heading-7 font-normal text-[#70747D]'>User info</p>
                                <Image
            ...
            {
                openDeleteModal && (
                    <ConfirmationModal isOpen={openDeleteModal} onClose={handleCloseDeleteModal} onConfirm={handleCloseDeleteModal} title='Delete user account?' message='Are you sure you want to delete this user? This action is permanent and cannot be undone. All associated campaigns, emails, and data will be permanently removed.' confirmText='Delete' cancelText='Go back' icon="/images/delete.png"/>
                )
            }
        </div>
    )
