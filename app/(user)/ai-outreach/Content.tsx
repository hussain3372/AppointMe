...
const Skeleton = () => (
  <motion.div className="space-y-3 w-full" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
    {[1, 2, 3].map((i) => (
      <motion.div
        key={i}
...
export default function Content() {
  ...
  const handleCopy = (messageId: string, content: string = '') => {
    navigator.clipboard.writeText(content || 'Thanks for trying this chatbot but its still in building process and will be available soon . Thanks for your patience')
      .then(() => {
  ...
  const handleRegenerate = () => {
    ...
    if (lastUserMessage) {
      ...
      const updatedMessages = messages.filter(msg => msg.type !== 'ai' || msg.id !== messages[messages.length - 1].id)
      ...
      setTimeout(() => {
        ...
        setMessages(prev => [...prev, { 
          id: `ai-${Date.now()}`, 
          type: 'ai', 
          content: 'Thanks for trying this chatbot but its still in building process and will be available soon . Thanks for your patience' 
        }])
      }, 2000)
    }
  }
    ...
    if (isImage) {
      ...
      reader.onload = (ev: ProgressEvent<FileReader>) => {
        ...
        if (typeof result === 'string') {
          setUploadedFile({ name: file.name, type: file.type, preview: result, isImage: true })
        }
      }
      reader.readAsDataURL(file)
    } else {
      setUploadedFile({ name: file.name, type: file.type, preview: null, isImage: false })
    }
  }
  ...
  return (
    <div className='h-screen p-5 flex flex-col justify-between sm:max-h-[calc(100vh-100px)] overflow-auto  bg-[#fef4ed]'>
              <button
                ...
                className="p-2 inline md:hidden bg-white  mt-3 w-[45px] rounded-md shadow-md border border-[#E2E3E5]"
              >
               <LucideMenu/>
              </button>
      ...
      <div className={`flex ${showIntro ? "h-1/2" : ""}  flex-col  max-h-[calc(100vh-300px)] sm:max-h-[calc(100vh-368px)] hide-scrollbar justify-end items-center`}>
...
<path d="M18.1667 15.666V5.66602M11.5 19.0994L12.3333 15.666H7.47502C7.21627 15.666 6.96109 15.6058 6.72966 15.4901C6.49823 15.3743 6.29693 15.2063 6.14168 14.9993C5.98644 14.7924 5.88152 14.5521 5.83523 14.2975C5.78895 14.0429 5.80257 13.7811 5.87502 13.5327L7.81668 6.86602C7.91766 6.51982 8.12819 6.21572 8.41668 5.99935C8.70518 5.78298 9.05607 5.66602 9.41668 5.66602H20.6667C21.1087 5.66602 21.5326 5.84161 21.8452 6.15417C22.1578 6.46673 22.3333 6.89065 22.3333 7.33268V13.9993C22.3333 14.4414 22.1578 14.8653 21.8452 15.1779C21.5326 15.4904 21.1087 15.666 20.6667 15.666H18.3667C18.0566 15.6662 17.7527 15.7528 17.4892 15.9162C17.2257 16.0797 17.013 16.3133 16.875 16.591L14 22.3327C13.607 22.3278 13.2202 22.2342 12.8685 22.0589C12.5168 21.8835 12.2092 21.6309 11.9688 21.32C11.7284 21.0091 11.5614 20.6479 11.4802 20.2633C11.3991 19.8788 11.4058 19.4809 11.5 19.0994Z" stroke="#414652" fill={`${isDisLiked?"black":""}`} stroke-linecap="round" stroke-linejoin="round"/>
              ...
              <AnimatePresence>
                {isLoading && (
                  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="flex justify-start">
                    <div className="bg-white shadow-md rounded-2xl px-4 py-3 w-64"><Skeleton /></div>
                  </motion.div>
                )}
              </AnimatePresence>
            </motion.div>
              ...
              {uploadedFile && (
                ...
                >
                  {uploadedFile.isImage && uploadedFile.preview ? (
                    <>
                      <img src={uploadedFile.preview} alt="Uploaded preview" className="w-full h-full object-cover" />
                      <AnimatePresence>
                        {isHoveringFile && (
                          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center">
                    ...
                    <div className="w-full h-full flex flex-col items-center justify-center bg-[#F5F5F5] border border-[#00000014] relative">
                      ...
                      <span className="text-[8px] text-[#70747D] truncate max-w-[56px] px-1">{uploadedFile.name}</span>
                      <AnimatePresence>
                        {isHoveringFile && (
                          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-black bg-opacity-50 rounded-lg flex items-center justify-center">
          ...
          <div className="flex justify-between">
            ...
            >
              <div className='bg-[#F87B1B] rounded-sm p-1'>
                <img src='/images/file.svg' alt='upload something' height={16} width={16} />
              </div>
