...
const LeadsGenerationGraph = () => {
  ...
  const getYAxisLabels = (): string[] => {
    ...
    if (activeView === "Today") {
      ...
      while (labels.length < 3) {
        ...
        const extraLabel =
          extraValue >= 1000 ? `${extraValue / 1000}k` : `${extraValue}`;
  ...
  return (
    <div
      className="w-full mx-auto h-[275px]"
      style={{
        borderRadius: 8,
    ...
    >
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-4">
        <h2 className="text-sm sm:text-base font-medium text-[#111827] text-center sm:text-left">
          Leads generation
        </h2>
        ...
        <div className="flex gap-1 sm:gap-2 flex-wrap justify-center sm:justify-start">
          {views.map((view) => (
            <button
              ...
              className={`flex-shrink-0 rounded-[100px] px-2 sm:px-3 py-1.5 text-xs sm:text-[12px] font-normal transition-colors whitespace-nowrap ${
                activeView === view
      ...
      <div className="relative flex-1">
        {/* Y-axis - Responsive */}
        <div className="absolute -left-2 top-0 bottom-1 flex flex-col justify-between heading-7 font-medium text-[#70747D] pr-2 sm:pr-4 w-6 sm:w-10">
        ...
        <div className=" ml-5 h-full flex flex-col">
          ...
          {/* X-axis */}
          <div className="flex ml-4 heading-7 text-[#414652] font-normal overflow-visible">
            {months.map((month, index) => (
              <div
                key={month}
                className="flex-1 text-center   -ml-3 px-0.5 rotate-month"
                title={month}
              >
                {month}
              </div>
