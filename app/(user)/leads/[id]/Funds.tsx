    ...
    <div className="py-5 px-5 w-full ">
      <div className="flex  items-start">
        ...
        <div className="flex flex-col leading-tight">
          <p className=".heading-4 font-medium text-[#111827]">Google</p>
          <p className="body-5 font-normal text-[#70747D]">google.com</p>
        </div>
      </div>
      ...
      <div className="pt-[24px] flex flex-col gap-2">
        {funds.map((item) => (
          <div
            key={item.id}
            className="bg-[#FEF4ED] rounded-lg w-full flex justify-between items-center p-3"
          >
      ...
      >
        <div className="flex gap-2 flex-col max-h-[500px] hide-scrollbar overflow-auto  pt-3">
          {company.map((item) => (
            <div
              key={item.id}
              className="flex items-center gap-4 border-b px-3 py-5 border-[#E2E3E5]"
            >
              <Image src={item.img} alt={item.title} height={20} width={20} />
              <div className="flex flex-col gap-1">
                <p className="heading-7 font-medium text-[#111827]">
                  {item.value}
                </p>
                <p className="heading-7 font-medium text-[#70747D]">
                  {item.title}
                </p>
              </div>
            </div>
